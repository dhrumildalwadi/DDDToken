var DDDToken = artifacts.require("./DDDToken");

contract('DDDToken', function(accounts) {
    var tokenInstance;
    it('initializes the contract with correct values', function (){
        return DDDToken.deployed().then(function(instance){
            tokenInstance = instance;
            return tokenInstance.name();
        }).then(function(name){
            assert.equal(name, 'DDD Token', 'has correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol, 'DDD', 'has correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'DDD Token v1.0', 'has correct standard');
        });
    });
    ...
    it('transfers token ownership', function() {
        return DDDToken.deployed().then(function(instance) {
            tokenInstance = instance
            return tokenInstance.transfer.call(accounts[1], 7777777777777777);
